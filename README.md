...
Практикум доповнює лекційний курс «Бази даних» наскрізним прикладом бази даних інтернет-магазину: від аналізу предметної області та побудови концептуальної моделі до маніпулювання даними, аналітичних SQL-запитів, транзакцій та документування проєкту.
...
- додавати, змінювати й видаляти записи зі збереженням цілісності;
- використовувати з'єднання, групування та віконні функції;
...
- застосовувати транзакції, індекси, представлення, процедури і тригери;
- виконувати контрольований імпорт та експорт даних;
- створювати документацію, достатню для розгортання та супроводу БД.

| Блок | Зміст | Результат |
|---:|---|---|
| 1 | Проєктування і створення БД | ER-модель, реляційна схема, DDL |
| 2 | Створення та маніпулювання даними | Відтворюваний тестовий набір, DML |
| 3 | Вибірка й аналіз даних | JOIN, агрегування, підзапити, CTE, віконні функції |
| 4 | Надійність і продуктивність | Транзакції, індекси, плани, контроль якості |
| 5 | Обмін і програмовані об'єкти | Імпорт, експорт, views, procedures, triggers |
| 6 | Документування проєкту | Технічна документація та звіт |

### Навчальні SQL-скрипти

Каталог [`sql`](sql/) містить послідовну програму практикуму. Один файл відповідає одній темі:

| Файл | Тема |
|---|---|
| [`01_create_database.sql`](sql/01_create_database.sql) | Створення БД, таблиць, ключів та обмежень |
| [`02_insert_data.sql`](sql/02_insert_data.sql) | Створення записів і тестовий набір даних |
| [`03_select_basics.sql`](sql/03_select_basics.sql) | `SELECT`, фільтрація, сортування, `CASE`, `NULL` |
| [`04_data_manipulation.sql`](sql/04_data_manipulation.sql) | `INSERT`, `UPDATE`, `DELETE` |
| [`05_joins.sql`](sql/05_joins.sql) | Внутрішні, зовнішні та self-з'єднання |
| [`06_aggregation.sql`](sql/06_aggregation.sql) | Агрегатні функції, `GROUP BY`, `HAVING` |
| [`07_subqueries.sql`](sql/07_subqueries.sql) | Вкладені й корельовані запити |
| [`08_cte_and_windows.sql`](sql/08_cte_and_windows.sql) | CTE та віконні функції |
| [`09_transactions.sql`](sql/09_transactions.sql) | Транзакції, блокування та конкурентний доступ |
| [`10_indexes_and_explain.sql`](sql/10_indexes_and_explain.sql) | Індекси та `EXPLAIN ANALYZE` |
| [`11_views_routines_triggers.sql`](sql/11_views_routines_triggers.sql) | Представлення, процедури і тригери |
| [`12_import_export.sql`](sql/12_import_export.sql) | Контрольований імпорт та експорт CSV |
| [`13_data_quality.sql`](sql/13_data_quality.sql) | Пошук аномалій і автоматизовані перевірки |

Спочатку виконайте файли `01` і `02`. Решту можна опрацьовувати послідовно. Файли `04` і `09` скасовують демонстраційні зміни через `ROLLBACK`; файл `10` створює постійний індекс, а файл `11` — постійні програмовані об'єкти. Системно залежні команди у файлі `12` навмисно закоментовані.
